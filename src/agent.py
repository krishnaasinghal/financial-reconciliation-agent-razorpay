...
def explain(result) -> str:
    if result.anomaly_type == "PARTIAL_SETTLEMENT":
        return f"The bank credit is below the deterministic expected net by INR {abs(result.difference or 0):,.2f}. This may reflect a reserve, split settlement, adjustment, or pending release."
    if result.anomaly_type == "MISSING_SETTLEMENT":
        return "The payment was captured, but no settlement record or bank credit is present yet."
    if result.anomaly_type == "DELAYED_SETTLEMENT":
        return "The amount agrees, but the settlement arrived outside the normal settlement window."
    if result.anomaly_type == "DUPLICATE_BANK_CREDIT":
        return "More than one bank credit references the same settlement and should be reviewed before posting."
    if result.anomaly_type == "UNMATCHED_BANK_CREDIT":
        return "This bank credit has no matching payment settlement in the synthetic batch."
    if result.anomaly_type:
        return f"The deterministic engine flagged {result.anomaly_type.replace('_', ' ').lower()} for review."
